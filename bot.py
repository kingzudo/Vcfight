...
# stream_key = user_id for custom, "default" for default account
active_streams = {}
...
async def start_command(client, message: Message):
    try:
        ...
        # ✅ UPDATED: Always show Default button even if already logged in
...
# ✅ FIXED: logout should work even if session terminated
async def force_logout_user(user_id: int):
    """
    Best-effort logout:
    - leave VC if playing
    - stop calls/client if possible
    - ALWAYS delete session file
    - cleanup dicts
    """
    # leave VC best effort
    try:
        if user_id in active_streams and user_id in user_calls:
    ...
    # stop pyrogram client best effort
    try:
...
async def logout_command(client, message: Message):
    try:
        ...
        if user_id in user_accounts or os.path.exists(f"/app/sessions/user_{user_id}.session"):
            await force_logout_user(user_id)
            await message.reply_text("✅ Logged out successfully! (force cleaned)")
        else:
...
async def callback_handler(client, callback_query):
    try:
        ...
        if data == "use_default":
            ...
            # ✅ fixed: force logout even if terminated
            await force_logout_user(user_id)
            await callback_query.message.reply_text("✅ Logged out successfully! (force cleaned) Use /start to login again.")
